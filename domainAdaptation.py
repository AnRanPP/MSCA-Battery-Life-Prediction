...
from models import CPGRU, CPLSTM, CPMLP, CPBiGRU, CPBiLSTM, CPTransformer, PatchTST, iTransformer, Transformer, \
    CACE_MLP, CACE_GRU, CACE_Transformer, \
...
for ii in range(args.itr):
    ...
    setting = '{}_sl{}_lr{}_dm{}_nh{}_el{}_dl{}_df{}_lradj{}_dataset{}_loss{}_wd{}_wl{}_bs{}'.format(
        args.model,
        args.seq_len,
        args.learning_rate,
        args.d_model,
    ...
    for epoch in range(args.train_epochs):
        ...
        for i in range(len(train_loader)):
            with accelerator.accumulate(model):
                cycle_curve_data, curve_attn_mask,  labels, life_class, scaled_life_class, weights, seen_unseen_ids = next(iterator_source)
                target_cycle_curve_data, target_curve_attn_mask,  target_labels, target_life_class, target_scaled_life_class, target_weights, target_seen_unseen_ids = next(iterator_target)
                model_optim.zero_grad()
                iter_count += 1
                
                scaled_life_class = scaled_life_class.float()
                cycle_curve_data = cycle_curve_data.float()
                curve_attn_mask = curve_attn_mask.float()


                target_life_class = target_life_class.float()
                target_cycle_curve_data = target_cycle_curve_data.float()
                target_curve_attn_mask = target_curve_attn_mask.float()
                
                

                # encoder - decoder
                outputs, embeddings = model(cycle_curve_data, curve_attn_mask, return_embedding=True)
                target_outputs, target_embeddings =  model(target_cycle_curve_data, target_curve_attn_mask, return_embedding=True)
