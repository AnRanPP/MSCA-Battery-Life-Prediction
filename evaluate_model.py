...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, required=False, default='Transformer',
                        help='model name, options: [Transformer, DLinear]')
    parser.add_argument('--seed', type=int, default=2021, help='random seed')

    # data loader
    parser.add_argument('--dataset', type=str, default='MIX_large', help='dataset description')
    ...
    parser.add_argument('--args_path', type=str,
                        default='F:\LXP\Project\PythonProject\BatteryLife\checkpoints\PCE_v2_sl50_lr3e-05_dm512_nh8_el3_dl2_df2048_lradjtype3_datasetMIX_large_lossMSE_wd5e-05_wlFalse_bs24_s2021-PCE_v2_Enhanced/',
                        help='the path to the pretrained model parameters')
    parser.add_argument('--eval_dataset', type=str, help='the target dataset')
    parser.add_argument('--eval_cycle_min', type=int, default=-1, help='The lower bound for evaluation')
    parser.add_argument('--eval_cycle_max', type=int, default=-1, help='The upper bound for evaluation')
    ...
    for ii in range(args.itr):
        ...
        if args.model == 'Transformer':
            ...
            model = PCE.Model(args).float()
        elif args.model == 'PCE_v2':
            # 强制设置正确的参数以匹配保存的模型
            args.d_model = 384
            args.d_ff = 512
            args.n_heads = 4
            ...
            print(f"设置参数: d_model={args.d_model}, d_ff={args.d_ff}, n_heads={args.n_heads}")

            # 然后是原来的模型初始化
            model = PCE_v2.Model(args).float()
        elif args.model == 'MSCAMLP':
            model = MSCAMLP.Model(args).float()
        ...
        if accelerator.check_trigger():
            ...
            if eval_cycle_min is None or eval_cycle_max is None:
                calculate_metrics_based_on_seen_number_of_cycles(total_preds, total_references, total_seen_number_of_cycles, alpha, alpha2, args.model, args.dataset, finetune_dataset=finetune_dataset, start=args.seq_len, end=args.early_cycle_threshold, seed=args.seed)
