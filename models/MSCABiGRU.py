# 保存为 F:\LXP\Project\PythonProject\BatteryLife\models\MSCABiGRU.py
...
class Model(nn.Module):
    """MSCA-BiGRU模型 - 使用MSCA插件替代CyclePatch"""
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, return_embedding=False):
        ...
        # Pack sequences for BiGRU
        packed_input = nn.utils.rnn.pack_padded_sequence(
            cycle_embeddings, lengths, batch_first=True, enforce_sorted=False
        )

        # Inter-cycle processing with BiGRU
        packed_output, h_n = self.inter_BiGRU(packed_input)
