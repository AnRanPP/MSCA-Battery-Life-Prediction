# F:\LXP\Project\PythonProject\BatteryLife\models\MSCA_Plugin_simple_fusion.py
...
class MSCABlock(nn.Module):
    """增强的MSCA块（此版本用于 'w/ Simple Fusion' 实验）"""
    ...
    def __init__(self, in_dim, hidden_dim, out_dim, drop_rate, use_conv=True):
        ...
        # 注意：尽管此实验不用门控，但为了保持代码结构一致，我们仍然保留AdaptiveGating的初始化
        if self.use_conv:
    ...
    def forward(self, x, raw_cycle_data=None):
        ...
        if self.use_conv and raw_cycle_data is not None:
            ...
            # --- 这是 "w/ Simple Fusion" 实验的核心逻辑 ---
            fused_features = linear_features + conv_features
            # --- 核心逻辑结束 ---
        else:
